...
type SortKey = 'due_date_asc'|'due_date_desc'|'amount_asc'|'amount_desc'
...
export default function BillsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setCategories(cats||[])
  },[filter,selectedMonth,sort])

  useEffect(()=>{load()},[load])

  function openNew() { setEditing(null); setError(''); setForm({title:'',amount:'',due_date:'',category_id:'',recurrence:'once',status:'pending'}); setShowModal(true) }
  function openEdit(b:Bill) { setEditing(b); setError(''); setForm({title:b.title,amount:String(b.amount),due_date:b.due_date,category_id:b.category_id||'',recurrence:b.recurrence||'once',status:b.status}); setShowModal(true) }
  ...
  async function save() {
    ...
    try {
      ...
      const payload = {title:form.title.trim(),amount:amt,due_date:form.due_date,status:form.status,category_id:form.category_id||null,recurrence:form.recurrence,payment_method:'pix'}
      if (editing) {
    ...
    } catch(e:any) { setError('Erro inesperado: '+String(e?.message||e)) }
    setSaving(false)
  }
  ...
  async function changeStatus(id:string,status:string) {
    await supabase.from('bills').update({status}).eq('id',id); setShowStatusMenu(null); load()
  }
  ...
  const SORT_OPTS:Array<{v:SortKey,l:string}> = [
    {v:'due_date_asc',l:'Venc. ↑ (mais próximo)'},
    {v:'due_date_desc',l:'Venc. ↓ (mais distante)'},
    {v:'amount_desc',l:'Valor ↓ (maior primeiro)'},
    {v:'amount_asc',l:'Valor ↑ (menor primeiro)'},
  ]

  return (
    <div style={{minHeight:'100vh',overflow:'hidden',width:'100%'}} onClick={()=>showStatusMenu&&setShowStatusMenu(null)}>
      <div style={{background:'#fff',borderBottom:'1px solid var(--border)',padding:'0 16px',height:56,display:'flex',alignItems:'center',justifyContent:'space-between',position:'sticky',top:0,zIndex:50}}>
        ...
        <button onClick={openNew} style={{padding:'7px 14px',background:'var(--accent)',color:'#fff',border:'none',borderRadius:8,fontSize:12,fontWeight:600,cursor:'pointer'}}>+ Nova</button>
      </div>

      <div style={{padding:'12px 14px'}}>
        {/* Filtros linha 1 */}
        <div style={{display:'flex',gap:6,marginBottom:8,overflowX:'auto',paddingBottom:2}}>
          <select value={selectedMonth} onChange={e=>setSelectedMonth(e.target.value)} style={{width:'auto',padding:'5px 8px',fontSize:11,fontWeight:600,borderRadius:8,border:'1.5px solid var(--border)',background:'#F8FAFC',flexShrink:0}}>
            {monthOpts.map(o=><option key={o.value} value={o.value}>{o.label}</option>)}
          </select>
          {[{v:'all',l:'Todas'},{v:'pending',l:'Pendentes'},{v:'overdue',l:'Vencidas'},{v:'paid',l:'Pagas'}].map(f=>(
            <button key={f.v} onClick={()=>setFilter(f.v)} style={{padding:'5px 10px',border:'1.5px solid',borderColor:filter===f.v?'var(--accent)':'var(--border)',borderRadius:20,fontSize:11,fontWeight:600,background:filter===f.v?'var(--accent-light)':'#fff',color:filter===f.v?'var(--accent)':'var(--muted)',cursor:'pointer',whiteSpace:'nowrap',flexShrink:0}}>{f.l}</button>
          ))}
        </div>
        {/* Filtros linha 2 - ordenação */}
        <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:12}}>
          ...
          <select value={sort} onChange={e=>setSort(e.target.value as SortKey)} style={{flex:1,padding:'5px 8px',fontSize:11,fontWeight:600,borderRadius:8,border:'1.5px solid var(--border)',background:'#F8FAFC'}}>
        ...
        {bills.length===0?(
          <div style={{textAlign:'center',padding:'48px 20px',color:'var(--muted)'}}>
            ...
            <button onClick={openNew} style={{padding:'8px 20px',background:'var(--accent)',color:'#fff',border:'none',borderRadius:8,fontSize:13,fontWeight:600,cursor:'pointer'}}>+ Adicionar</button>
          </div>
        ):(
          <div style={{background:'#fff',border:'1px solid var(--border)',borderRadius:12,overflow:'hidden'}}>
            {bills.map((b,i)=>{
              ...
              return (
                <div key={b.id} style={{padding:'11px 14px',borderBottom:i<bills.length-1?'1px solid #F8FAFC':'none',background:isOverdue?'#FFFAFA':'#fff'}}>
                  <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:8}}>
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      <div style={{fontSize:10,color:'var(--muted)',marginTop:2}}>
                        ...
                        {b.recurrence==='monthly'&&<span style={{marginLeft:5,fontSize:9,background:'#EFF6FF',color:'var(--accent)',padding:'1px 5px',borderRadius:8,fontWeight:600}}>Mensal</span>}
                        {isOverdue&&<span style={{marginLeft:5,fontSize:9,background:'var(--danger-light)',color:'var(--danger)',padding:'1px 5px',borderRadius:8,fontWeight:700}}>VENCIDA</span>}
                      </div>
                    </div>
                  ...
                  <div style={{display:'flex',gap:5,marginTop:8,flexWrap:'wrap'}} onClick={e=>e.stopPropagation()}>
                    <div style={{position:'relative'}}>
                      <button onClick={()=>setShowStatusMenu(showStatusMenu===b.id?null:b.id)} style={{padding:'3px 9px',background:si.bg,color:si.color,border:`1px solid ${si.color}33`,borderRadius:20,fontSize:10,fontWeight:700,cursor:'pointer'}}>{si.l} ▾</button>
                      {showStatusMenu===b.id&&(
                    ...
                    <button onClick={()=>openEdit(b)} style={{padding:'3px 9px',background:'#F1F5F9',color:'#475569',border:'1px solid var(--border)',borderRadius:20,fontSize:10,fontWeight:600,cursor:'pointer'}}>✏ Editar</button>
                    <button onClick={()=>deleteBill(b.id)} disabled={deleting===b.id} style={{padding:'3px 9px',background:'var(--danger-light)',color:'var(--danger)',border:'1px solid #FECACA',borderRadius:20,fontSize:10,fontWeight:600,cursor:'pointer'}}>{deleting===b.id?'…':'🗑 Excluir'}</button>
                  </div>
                </div>
              )
            })}
      ...
      {showModal&&(
        <div onClick={e=>{if(e.target===e.currentTarget){setShowModal(false);setError('')}}} style={{position:'fixed',inset:0,background:'rgba(15,23,42,.5)',zIndex:200,display:'flex',alignItems:'flex-end',justifyContent:'center'}}>
          <div style={{background:'#fff',borderRadius:'16px 16px 0 0',width:'100%',maxWidth:540,padding:'18px 16px 32px',maxHeight:'92vh',overflowY:'auto'}} onClick={e=>e.stopPropagation()}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14}}>
              <div style={{fontSize:15,fontWeight:700}}>{editing?'Editar Conta':'Nova Conta a Pagar'}</div>
              <button onClick={()=>{setShowModal(false);setError('')}} style={{background:'#F1F5F9',border:'none',borderRadius:8,width:28,height:28,cursor:'pointer',fontSize:14}}>✕</button>
            </div>
            ...
            {[{k:'title',l:'Nome da Conta *',ph:'Ex: Aluguel, Energia...'}].map(f=>(
              <div key={f.k} style={{marginBottom:12}}>
                ...
                <input value={form[f.k as keyof typeof form]} onChange={e=>setForm(p=>({...p,[f.k]:e.target.value}))} placeholder={f.ph} autoFocus={f.k==='title'}/>
              </div>
            ))}
            ...
            <div style={{display:'flex',gap:8}}>
              <button onClick={()=>{setShowModal(false);setError('')}} style={{flex:1,padding:12,border:'1.5px solid var(--border)',borderRadius:10,background:'#fff',cursor:'pointer',fontSize:13,fontWeight:600,color:'var(--muted)'}}>Cancelar</button>
              <button onClick={save} disabled={saving} style={{flex:2,padding:12,background:saving?'#93C5FD':'var(--accent)',color:'#fff',border:'none',borderRadius:10,cursor:saving?'not-allowed':'pointer',fontSize:13,fontWeight:700}}>
